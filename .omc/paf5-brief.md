...
## 실기 성공 (2026-07-21) — EAS 없이 커뮤 진단·수리 완료
**모델 실증**: 인에이블마다 MF=0x80 100% 재현 → cos δ<0 진단 → **180° 플립 CA[7] 322→−446**
→ 인에이블 버팀·**i_ba=1.184A 검출**·방향+1·expect-slip=slip(δ<45°). 서명 판정 YELLOW(잠정,
K_a 미측정 — 풀 Phase 2 GREEN이 베이스라인 확립). 안전천장 1.30A 클램프 실기 로그로 재확인.
**중요 — δ는 이 모터에서 결정론적**(CA[17]=5, CA[7]+절대엔코더): 전원 재인가로 재추첨되지 않음.
원장의 "전원마다 재추첨"은 다른 모터 얘기 → 정정 기록함.
**⚠️ 현재 CA[7]=−446은 RAM only(SV 안 함). 전원 끄면 322(나쁜 값)로 원복.**
복구지점: .omc/state/field_r0_snapshot_1784615249.json (CA[7]=322, 게인·리밋 포함).
미해결 설계결함 2건(원장 기록): ① 커뮤런의 P2_LIMITS 트랜잭션이 자기 CA[7] 쓰기를 차단
(자기 데드락) ② S0의 MF≠0 거부가 MF=0x80(자기가 고칠 폴트)에도 걸림. 둘 다 오프라인 수정 과제.
다음: Phase 2로 베이스라인 확립 → 서명 GREEN 승격 → 정격 조그.
