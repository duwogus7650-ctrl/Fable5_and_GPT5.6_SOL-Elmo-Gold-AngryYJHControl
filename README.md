# AngryYJH Control — Elmo Gold Twitter 제어·튜닝 도구

Made by 여재현 (SPG). Elmo **Gold Twitter** 서보드라이브를 위한 자체 PyQt6 데스크톱 제어
프로그램. Elmo Application Studio III(EAS III)의 선택 기능을 정품 Drive .NET Library와
실기 오라클로 구현·검증한다. 전체 EAS 기능 동등성은 현재 감사 중이며 주장하지 않는다.

> **2026-07-19 현재 `EAS LIVE PARITY AUDIT`를 처음부터 다시 수행 중이다.
> 기존 slice의 “완료”는 해당 로컬 구현/시험의 종료를 뜻했으며 EAS 기능 parity 완료가 아니다.
> 결정적 현황은 [`docs/eas-live-parity-audit-2026-07-19.md`](docs/eas-live-parity-audit-2026-07-19.md)를
> 우선한다. 현재 활성 구현 범위는 Quick Tuning + 제한형 Single Axis Motion +
> Expert Candidate Lab v2 + 로컬 Evidence/Page Status/User Units formula/
> Limits·Protections/Application Settings/Hidden Verification–Bode/Verification–Time
> + Single Axis Current Reference read-only snapshot + EAS-shaped five
> Current local drafts (output locked)
> documented-map inspector이다. 독립 안전 검토의
...
> 최신 SR23 + PX/PU + Current preset revision의 전체 회귀는
> **1964 passed in 636.67s, exit 0**이다.
...
| 화면 | 내용 | 상태 |
|---|---|---|
| **Motion** | raw PX 텔레메트리 + 별도 PU(EAS/0x6064) 좌표, generation-bound Session Zero, 제한형 finite PTP, Single Axis Controls 문서 권한 map, UM/Digital Input 6채널/Digital Output 4채널, Drive Current readback, EAS형 Current local draft 5개 | EAS 표시=`PU`, raw=`PX`, 관찰 `PU-PX=2^25`; 원점 원인은 `NEED-DATA`이고 자동 보정하지 않음. Current Set TC는 5개 모두 항상 잠김; UM/TC assignment·output actuation·physical-pin/실전류/토크 판정 없음 |
...
- 다섯 번째 `USER UNITS` 단계는 NetHelp의 DS-402 위치 식을 명시적
  `FC[1,2,5,6,7,8]`로만 exact `Fraction` 계산한다. MAN-G-CR product guard의
  인덱스 묶음이 식과 다른 점은 `DOCUMENTED GROUPING MISMATCH · PURPOSE NEED-DATA`로
  유지한다. 현재 drive 자동 채움, FC/OF write, Apply/SV, Motion/Recorder/Status
  단위 전파는 없다. 상세 계약:
  [`docs/expert-user-units-v0.1.md`](docs/expert-user-units-v0.1.md).
- 여섯 번째 `LIMITS / PROTECT` 단계는 `MC/BV/PL/CL/US`, `SD/VH/VL/XM/XA`,
  `ER/XP/LL/HL`의 문서상 의미·단위·access와 충돌을 frozen catalog로 보여준다.
  로컬 immutable catalog만 GREEN이며 current drive config, EAS parity,
  protection efficacy, 안전 판정과 read/write/Apply/SV는 `NEED-DATA / NO-GO`다.
  focused 회귀 69 passed, 전체 오프라인 회귀 1513 passed와 최신 Python 3.14
  runtime smoke를 완료했다. 초기 표의 흰 배경/밝은 글자 대비 결함도 재현해
  세 테마 전용 스타일과 `>=4.5` contrast 회귀로 수정했다.
  상세 계약:
  [`docs/expert-limits-protections-evidence-v0.1.md`](docs/expert-limits-protections-evidence-v0.1.md).
- 일곱 번째 `APP SETTINGS` 단계는 `Brake`, `Settling Window`,
  `Inputs and Outputs`의 4/4/5개 행을 immutable documented map으로 보여준다.
  authority는 `DOCUMENTED_APPLICATION_SETTINGS_MAP_ONLY`, status는
  `PARTIAL_NEED_DATA`다. 24개 source identity, 9개 문서 충돌, 16개 고정 경고,
  6개 missing-evidence 항목을 보존하며 로컬 inspect만 GREEN이다.
  `IP + IB[N]`와 `GO[N] + OP` 상태 행은 `unavailable · not sampled`이며
  현재 drive/I/O/brake 상태가 아니다. drive read/worker/command/write,
  Apply/Revert/SV, output actuation과 motion은 모두 없다. focused 회귀
  **85 passed**, 전체 repository 회귀 **1529 passed in 476.14s**를 직접
  종료코드 0으로 완료했다. 독립 검토는 HIGH/MEDIUM/LOW 지적이 없고 24개
  source SHA-256이 모두 일치했다. Python 3.14, 1366×820,
  `OFFLINE · READ ONLY` runtime에서 4/4/5개 행과 짧은 표 헤더를 확인했으며
  Connect나 drive/worker/command/output/motion I/O는 실행하지 않았다.
  상세 계약:
  [`docs/expert-application-settings-evidence-v0.1.md`](docs/expert-application-settings-evidence-v0.1.md).
- 여덟 번째 `BODE DOC MAP` 단계는 EAS에서 기본적으로 숨겨진
  Current와 Velocity/Position Bode 검증 페이지, 그리고 Tuner Verification
  설정을 8/8/8개 immutable documentation row로 보여준다. authority는
  `DOCUMENTED_HIDDEN_BODE_MAP_ONLY`, status는 `PARTIAL_NEED_DATA`다.
  7개 source identity, 4개 문서 충돌, 12개 고정 경고, 8개 missing-evidence를
  보존한다. 기존 P1 Bode preview는 OFFLINE MODEL이고 이번 화면은 실제 EAS
  field experiment의 문서 지도이므로 서로 같은 결과로 표시하지 않는다.
  actual Current Verify는 ENERGY, V/P Verify는 MOTION이지만 이 화면에는
  실행·설정변경·recording·drive I/O control이 없다. pure model 14 passed,
  Expert 영향범위 회귀 **211 passed in 65.06s**, 전체
  **1547 passed in 503.66s / exit 0**과
  7개 설치 source SHA-256 재대조를 완료했다. Python 3.14, 1366×820,
  `OFFLINE` runtime에서 8/8/8개 행, action/edit widget 0, 수평 scroll 0을 확인했다.
  상세 계약:
  [`docs/expert-bode-verification-evidence-v0.1.md`](docs/expert-bode-verification-evidence-v0.1.md).
- 아홉 번째 `TIME DOC MAP` 단계는 Current Verification–Time,
  Velocity/Position Recording Setup, Velocity/Position Verification–Time을
  각각 8개 grouped row의 immutable documentation map으로 보여준다.
  authority는 `DOCUMENTED_TIME_VERIFICATION_MAP_ONLY`, status는
  `PARTIAL_NEED_DATA`다. 화면을 열거나 section을 바꿔도 drive read,
  Recorder 구성·acquisition, Verify, Enable, PTP, Jog, Sine/Step injection,
  command/write/Apply/SV, energization 또는 motion을 실행하지 않는다.
  실제 Current Verification–Time은 ENERGY이며 EAS motor-move 경고처럼
  축이 움직이거나 twitch할 수 있고, 실제 V/P Verification–Time은 editable
  current, PTP/Jog/Sine/Step, field weakening/friction compensation을 포함한
  MOTION 경로이므로 모두 `NEED-DATA / NO-GO`다. 전체 회귀는
  **1567 passed in 524.64s**이며, 독립 검토의 HIGH/MEDIUM/LOW 잔여 finding은
  없다. 설치 source SHA-256은 **8/8 일치**했다. Python 3.14,
  1366×820 새 창 `OFFLINE · READ ONLY` runtime에서 세 section 각각 8 groups,
  실행 제어 0개를 확인했다.
  상세 계약:
  [`docs/expert-time-verification-evidence-v0.1.md`](docs/expert-time-verification-evidence-v0.1.md).
...
- `expert_user_units.py` — explicit FC 입력의 exact DS-402 documented formula
  preview와 grouping-mismatch 경계.
- `expert_limits_protections_evidence.py` — no-I/O immutable Limits/Protections
  documented parameter map과 문서 충돌·위험 경고.
- `expert_application_settings_evidence.py` — no-I/O immutable Brake/Settling/I/O
  documented map, source identity와 fail-closed capability.
- `expert_bode_verification_evidence.py` — hidden Current/V/P Bode와 Tuner
  Verification 설정의 no-I/O immutable documented map.
...
- `single_axis_current_reference.py` — `MO/SO/MF/SR` pre/post와
  `UM/TC/IQ/ID/CL[1]/PL[1]/LC/MC` query만 사용하는 identity-bound
  Current Reference decoder/reader. `TC=` command, enable, loop 변경,
  energization과 motion authority는 제공하지 않는다.
...
- `docs/single-axis-current-reference-read-v0.1.md` — Current Reference의
  16-query 순서, source hash, `SR/MO/SO/LC` 교차검증, 현재 target
  readback과 잠긴 `TC=` command 경계를 기록한다.
...
- `docs/expert-limits-protections-evidence-v0.1.md` — Current/Motion Limits와
  Protections의 immutable 문서 map, conflict와 zero-read/write 경계.
- `docs/expert-application-settings-evidence-v0.1.md` — Brake/Settling/I/O의
  immutable 문서 map과 zero-read/actuation/motion 경계.
- `docs/expert-bode-verification-evidence-v0.1.md` — hidden Bode page와 Tuner
  Verification 설정의 static map, MODEL/field 분리와 ENERGY/MOTION 실행 gap.
...
PU=-2,004,825,502 user units, `PU-PX=2^25`, XM=0, FC=1,
CA[45]=1, VX=0.000 cnt/s를 관찰했다. 확장된 28-query revision은
**59.1 ms**로 live 재측정했다. assignment, `BG`, `MO=1`,
...
신규 PX/PU·Current preset 집중 계약은 **142 passed**, 직접 영향 범위는
**284 passed in 133.72s**, EAS parity ledger는 **5 passed**다. 전체
repository는 최신 **1964 passed in 636.67s, exit 0**으로 통과했다.
...
## Latest SR status correction · live bit 23

같은 Read Only 세션의 admitted Axis Summary에서 `SR=0x0080C000`이
관찰됐다. 이전 decoder는 `0x00800000`(bit 23)을 reserved로 오판해
Single Axis status와 Enable projection을 `UNKNOWN`으로 만들었다.
설치 EAS Gold SR 도움말에 정의된 bits 21/22/23/27/30을 반영하고
`MO↔SR22` redundancy를 추가했다. bit 23은 firmware/source 차이를 고려해
물리 motion verdict가 아닌 source-bound `movement/standstill indication`으로만
표시한다. bit 27은 STO diagnostics error로서 Enable 정상 상태보다 우선하며
`FAULT REPORTED - NO AUTO-RETRY`로 차단한다.

첫 28-query 시도의 SR transition은 당시 raw pre/post가 남지 않아 정확한 bit가
`UNVERIFIED`다. 이후 오류에는 두 SR 값과 changed bit 번호를 보존하며,
mask를 약화하지 않았다. 상세:
