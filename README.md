...
- **Phase 1 Apply 소프트웨어 가드 완료**: KP[1]/KI[1]을 평문 소수 6자리 이하로 전송하고,
  반올림 오차·무성 0 저장·되읽기 불일치를 차단한다. 두 게인 모두 검증된 뒤에만 SV를 허용한다.
  사고 재현 목과 전체 166개 테스트는 통과했으며, 실제 드라이브 Apply 재검증은 아직 감독 실기 전이다.
