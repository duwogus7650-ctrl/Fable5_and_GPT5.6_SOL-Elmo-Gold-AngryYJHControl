...
> **2026-07-17 현재 활성 범위는 Quick Tuning + 제한형 Single Axis Motion으로 축소됐으며,
> 독립 확인 P1 blocker 4건 때문에 merge와 hardware 사용이 차단된 Private Draft 인계 상태다.**
> 현재 구현·증거·중단 지점·재개 순서는
> [`docs/current-scope-handoff.md`](docs/current-scope-handoff.md), 실시간 표시용 요약은
> [`tasks/status.md`](tasks/status.md)를 기준으로 한다. `.omc/paf5-brief.md`와 전체 EAS 계획은
...
| **Tuning** | Phase 1/2 공통 RAM 임시 적용→복원/되읽기→별도 SV, connection-bound 커뮤테이션 서명 | 과거 LIVE 이력 · 최신 fail-closed 코드 OFFLINE · 현재 리비전 실기 대기 |
...
- **Phase 1 적용도 트랜잭션 방식으로 통일**: `Apply P1 → RAM`이 기존 KP[1]/KI[1]을 먼저
  스냅숏하고 새 값을 RAM에만 쓴다. `Restore P1 → Original`은 원값 전부를 복원·되읽기하고,
  `Save P1 → SV`는 저장 직전 두 게인이 임시 적용값과 모두 일치할 때만 SV를 보낸다.
  평문 소수 6자리, 반올림 오차, 무성 0 저장, 부분 쓰기 실패를 차단하는 전체 회귀시험을 통과했으며
  실제 드라이브의 신규 P1 트랜잭션 흐름은 아직 감독 실기 전이다.

### 과거 실기 이력 — 자체 속도/위치 오토튠 (Phase 2) + 검증런 (2026-07-15)

**측정 → 게인 산출 → 적용 → 검증까지 EAS 없이 앱 안에서 완결.**

- **P2 적용은 트랜잭션 방식**: `Apply P2 → RAM`이 현재 KP[2]/KI[2]/KP[3]을 먼저
  스냅숏하고 새 값을 RAM에만 쓴다(`SV` 없음). `Run Verify`가 GREEN일 때만
  `Save P2 → SV`가 열리며, 저장 직전 세 게인을 다시 읽어 임시 적용값과 모두 일치해야 한다.
  검증 YELLOW/RED/Abort는 원래 게인을 자동 복원하고, 중간 쓰기 실패도 전 값 복원을 시도해
  되읽기로 확인한다. 복원을 확인할 수 없으면 RED 상태와 재시도용 스냅숏을 유지한다.
...
- `SV` 응답 유실은 저장 성공/실패를 추측하지 않고 `UNKNOWN`으로 잠근다. 같은 링크에서는 새
  P1/P2 trial, 설정 쓰기, enable/motion, 반복 `SV`가 차단된다. P1/P2는 SV 직전, Motor는
  첫 RAM assignment 전에 identity·VR/VP/VB·connection epoch와 원래/적용 profile을
  interprocess-serialized durable ledger에 기록하므로 강제 앱 종료 뒤에도
  잠금이 유지된다. 해제는 현장 냉간 OFF→ON 확인 + 새 세션의 동일 target + 2회 query-only
  readback으로 durable 게인 프로필이 확인된 때만 가능하다. 이 결과는 특정 SV의 인과나
...
- **서명 전용 경로(2026-07-15 추가)**: `Run Commutation Signature (≤1.30 A)`는 +TC를
  ...
  **실기 GREEN: i_ba=0.679 A, 방향 +1,
  종료 MO=0·TC=0**.
- **안전 장치**: K_a 열화 조기검출(직전 GREEN의 0.5× 미만 → RED),
  RAM 적용·복원·SV 직전 전체 되읽기 검증(불일치·0·음수면 SV 금지),
  UM3 드래그 판별(기계 vs 커뮤 구분).
...
- `docs/persistence-audit.md` — P1/P2/Motor write-ahead 원장과 전원 재인가 후 query-only 판정 계약.
- `.omc/paf5-brief.md` — 상세 연속성 로그(모든 결정·수치·실기 발견).
- `vendor/elmo-downloads/` — 정품 .NET 라이브러리·펌웨어·문서(사용자 다운로드).

## 안전

설정 저장의 목표 계약은 MO=0 게이트 + 변경 전 원본 + RAM 되읽기/복원 + durable WAL + 단일 SV다.
그러나 현재 Quick Tuning은 독립 확인 P1 blocker 4건 때문에 restore·limit·검증 전 Save 계약을 아직
충족하지 못해 merge와 hardware 사용이 차단돼 있다. Feedback direct save도 versioned registry 전까지
잠겨 있다. Abort는 disable·복원을 시도하지만 즉시 기계 정지나 완전한 원상복원을 증명하지 않는다.
**무인 통전 금지.**
