# P1_CONFIG/P2_LIMITS/Motor active persistence audit + legacy P2 record 계약

## 목적과 범위

`SV`는 비휘발 파라미터를 RAM에서 flash로 저장하며 통신을 수백 ms 중단할 수 있다. 따라서
명령을 보낸 뒤 응답을 잃으면 저장 성공과 실패를 응답만으로 구분할 수 없다. 이 기능은 그
상태를 추측하지 않고, 앱 재시작 뒤에도 쓰기 권한을 잠근 채 전원 재인가 후 실제 임시 구성,
legacy P2 게인 record 또는 Motor profile을 읽어서 분류한다. 같은 durable 원장은 `SV` 응답
유실뿐 아니라 P1 임시 구성과 P2 critical-limit의 RAM rollback을 증명하지 못한 경우도 보존한다.

Active production 범위는 P1_CONFIG `KP[1]/KI[1]/UM/SC[8]/CA[42..44]/CA[70]/SE[1..7]`,
P2_LIMITS `SD/HL[2]/LL[2]/ER[2]`, Motor `PL[1]/CL[1]/VH[2]/CA[19]/CA[28]` 프로파일이다.
P2 `KP[2]/KI[2]/KP[3]` record의 audit/복구 엔진은 legacy/offline incident 호환을 위해 남지만,
현재 production UI/domain은 durable pre-assignment gain-trial WAL이 없어 새 P2 gain trial과 Save를
drive I/O 전에 거부한다. P1 Apply/Save도 같은 WAL 부재와 verifier 부재로 잠겨 있다. 결과는 whole-drive 설정,
...
GUI는 요청을 queue에 넣는 순간 Motor 저장 버튼을 잠그고 과거 P1/P2 결과의 권한을 폐기한다.
연결/Motor 변경마다 단조 증가 세대 토큰을 발급하며, 현재 worker·현재 dispatch·같은 세대만
installed-gain Verify 또는 retained recovery를 진행할 수 있다. 후보 결과와 Verify verdict는
production gain Apply/Save 권한을 만들지 않는다. Motor 저장과 튜닝 dispatch는 양방향으로 동시에
시작할 수 없고, 재연결도 이전 세대의 결과와 지연된 worker 신호를 계승하지 않는다. 현재 worker의
결과 뒤에도 연결이 유효하고 persistence lock이 없을 때만 Motor 저장 버튼을 다시 연다.
...
2. record 종류별 original/applied 전체가 exact register set과 type/range 계약을 만족하는지 검증한다.
   P1_CONFIG/P2_LIMITS/Motor의 원시 명령·응답은 binary float로 바꾸기 전에 `Decimal`로 판정한다.
   float 반올림으로 frozen profile 또는 exact zero처럼 보이는 sub-ULP 값은 authority를 얻지 못한다.
3. Legacy P2 gain record는 0.1% 판정 허용오차, Motor와 임시 config phase는 정수/config를 포함한 exact
   계약으로 original/applied가
   구별되는지 확인한다. 구별되지 않으면 no-op으로 보고 `SV` 준비 자체를 거부한다.
4. P1_CONFIG/P2_LIMITS의 forward와 rollback capability는 분리한다. forward는 bounded/applied
   값만 허용하고, link가 같은 session에서 `MO/SO/VX=0`을 직접 확인한 단방향 rollback transition
   뒤에는 frozen original 값만 허용한다. 전환 뒤 forward/motion authority는 다시 발급하지 않는다.
5. Legacy/offline P2 gain engine은 `SV` 직전 `PERSISTING`을 만든다. 현재 production은 이 경로에
   들어갈 새 gain trial을 만들지 않는다. P1_CONFIG/P2_LIMITS/Motor는 첫 RAM
   쓰기 전 `RAM_APPLYING` record를 만든다. Motor만 전체 applied readback 뒤
   `PERSISTING`으로 전이한다. 모든 상태 변경은
...
6. 위 단계가 성공한 같은 identity·connection epoch 및 정확히 같은 record ID에서만 `SV`를 한 번 실행한다.
7. 같은 호출에서 실제 성공 응답을 받은 attempt만 resolved archive로 닫는다. 응답 유실이나 archive 실패는 active UNKNOWN으로 남긴다.
...
P1_CONFIG와 P2_LIMITS는 임시 RAM configuration이므로 전원 재인가 뒤 applied profile이 보여도
성공으로 해제하지 않는다. 동일 identity의 새 epoch에서 운영자 reset attestation, 두 번의 안정된
disabled query와 **original profile exact match**가 모두 확인될 때만
`ORIGINAL_PROFILE_AFTER_RESET`으로 해제한다. applied/mixed/1-count mismatch/불안정 read는
`TEMPORARY_CONFIGURATION_AFTER_RESET` 또는 UNKNOWN으로 계속 잠근다.
