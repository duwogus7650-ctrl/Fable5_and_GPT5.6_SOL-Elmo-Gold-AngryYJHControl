...
초기 구현 집중 회귀는 **127 passed in 60.34s**다. 이 수치는 decoder/transport/
worker/UI/catalog의 offline contract 증거이며 current drive readback이나 field
I/O validation 점수가 아니다. 전체 repository 결과와 runtime 확인은 최종
closeout에서 최신 값으로 갱신한다.
...
1. 최신 worker allowlist 수정 포함 전체 repository 회귀
2. EAS I/O Status Area와 같은 순간의 값 비교
3. 비통전 상태에서 known inactive/active 입력을 하나씩 독립 계측
4. polarity/filter/sticky와 source timestamp 의미 검증
5. 다른 feedback motor가 아니라 I/O count가 다른 Gold target에서 재발견
6. Digital Output은 별도 safe-state/부하/rollback/권한 설계 전까지 잠금
