...
**R=2점 DC 차분법**, **L=정현여진 |Z| 크기법**. 게인은 캘리브레이션 상수 2개로 산출. 모션은 정렬 스냅(≤11.25°기계각 1회)뿐.
...
## 3. R/L 측정
**3.1 UM=3 근거(전센서호환)**: UM=3은 피드백 센서 없이 개루프 전기각으로 전류 인가(p298). 어느 센서든 R/L 경로는 센서 미사용. 센서는 (a)모션감시 임계 환산(PX,CA[18]) (b)백업에만 등장 → 전부 read_feedback 값으로 파라미터화, **센서ID 하드코딩 금지**. 정렬 스냅 최대 반전기주기=180/16극쌍=11.25°기계각; 정렬후 d축→토크0→무모션.
...
**모션임계(센서파라미터화)**: 정렬허용=±(CA[18]·11.25/360+20%)counts; θ_abort=max(4, CA[18]·2°/360)(홀 96 저해상도 바닥4).
...
B4 정렬램프 TC→i_frac_low·CL[1] 10단계×100ms; wait 1s; PX 2회폴 200ms→|ΔPX|≤θ_abort
...
E3 [사용자액션 "적용"] MO==0 확인→KP[1]=;KI[1]=→(선택)SV
...
SC[8]≠0→0클리어(복원원값). 정렬스냅>±11.25°×1.2→abort. MF=0x200000(스턱)→abort+세그먼트단축안내. LC==1(포화)→abort.
