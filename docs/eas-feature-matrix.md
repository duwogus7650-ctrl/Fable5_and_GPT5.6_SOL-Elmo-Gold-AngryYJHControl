...
| Expert Tuning | Axis; Motor/Feedback; User Units; Limits/Protections; Application Settings; Current; Commutation; Velocity/Position; Summary | v2는 Current P1 → Velocity/Position P2 순수 offline Candidate Lab과 documented Evidence, Local Page Status, User Units formula, Limits/Protections, Application Settings, Hidden Verification–Bode, Verification–Time 및 Summary transaction map의 열 단계를 제공한다. 공통 P1/commutation/P2/installed Verify, candidate/installed readback과 EAS Enter/Apply state는 분리하며 아래 세부 차이는 backlog |
| Expert 세부 | `Current Limits`; `Motion Limits and Modulo`; `Protections`; `Brake`; `Settling Window`; `Inputs and Outputs`; Current `Identification / Design / Verification-Time / hidden Verification-Bode`; Commutation; Velocity/Position `Identification / Design / Scheduling / Verification-Time / hidden Verification-Bode`; Summary | P2 v2는 explicit `K_a/B`, count/s·peak-A basis, `GS[2]=0` single-point MODEL을 구현. Filter/Scheduling v0.1은 documented topology만, Page Status v0.1은 기존 local object 상태만 inspector로 제공. User Units v0.1은 DS-402 위치 식 하나의 explicit-input `PARTIAL/SCREENING`이다. Limits/Protections, Application Settings, Hidden Bode와 Verification–Time v0.1은 static documented map과 충돌/위험/누락만 제공한다. Hidden Bode page visibility/current EAS settings, actual Current ENERGY와 V/P MOTION Verify/acquisition/recording, current-drive/I/O/brake/Recorder readback, 값 검사·추천, protection efficacy, exact transaction/write/actuation, exact filter evaluator, Summary recommendation, EAS page icon/Enter/Apply/import/export parity는 미구현 |
| Motion - Single Axis | Position/position error/velocity/active current/last fault/status/program status; Digital In/Out; `STO1/STO2/ERR`; Drive Mode(UM); Enable; `Current / Stepper / Sine Reference`; PTP Absolute/Relative; Terminal/Command Reference; 2-chart Recorder | core telemetry, session zero, locked finite PTP, 별도 Recorder와 `MO/SO/MF/PS/SR/MS` 기반 Safety Snapshot MODEL projection 구현. `Documented Authority Map v0.1`은 3 section/12 row에서 각 control의 risk를 inspect-only로 분리한다. Drive Mode는 explicit read-only `UM` one-query snapshot, Digital Input 1–6은 `IL/IF/IP`, Gold Twitter Digital Output 1–4는 `OL/GO/OP`, Current Reference는 `TC/IQ/ID/CL[1]/PL[1]/LC/MC`, Position/Velocity References는 `PA[1]/PR[1]/JV/SP[1]/AC[1]/DC/SD/PX/VX`와 `MO/SO/MF/SR` pre/post의 bounded snapshot으로 구현했다. Enable/Disable v0.1은 existing Axis Summary를 재사용해 Disabled/Enabling/Enabled/Disabling-Brake-Hold/Fault/Unknown을 zero-new-I/O로 구분하며, `motor.enable`은 visible `LOCKED/NEED-DATA`, Disable은 기존 `drive.stop`만 사용한다. `UM=`/`TC=`/`PA/PR/JV=` assignment와 `BG`, sticky clear, mapping/filter write, output assignment/actuation, standalone `MO=1`, mode별 수동 구동, terminal, EAS recorder docking parity는 계속 미구현/NEED-DATA |
...
| Expert filter / gain scheduling | 세 번째 LOCAL inspector에서 type 0..6, controller KV slot, GS[2] 0..66 category, KG block을 탐색; KV/GS/KG emulation/write 없음 | `DOCUMENTED TOPOLOGY ONLY · PARTIAL`; MAN-G-CR 1.406 SHA-256 고정, KV/KG/GS 문서 충돌 5건 보존 | SimplIQ §15.4, B01G parity와 exact transfer/discretization/range/interpolation oracle 확보 |
| Expert Local Page Status / Errors v0.1 | 네 번째 LOCAL inspector에서 현재 P1/P2/evidence를 `MISSING/BLOCKED/STALE/INVALID/CURRENT LOCAL MODEL/DOCUMENTED PARTIAL`로 분류 | `LOCAL STATUS ONLY · PARTIAL`; installed/dispatch/Verify/Apply/Save 불변, poison-I/O와 exact P1↔P2 binding 음성 대조 | EAS idle/changed/warning/error 아이콘, Enter/Apply/Revert, last-page 저장, Summary recommendation과 drive readback은 별도 `NEED-DATA` |
| Expert User Units · Documented Formula v0.1 | 다섯 번째 LOCAL page에서 explicit `FC[1,2,5,6,7,8]`로 NetHelp DS-402 위치 식을 exact `Fraction` 계산; MAN-G-CR 두 product guard는 원문 grouping 그대로 별도 적용 | `DOCUMENTED_FORMULA_PREVIEW · PARTIAL/SCREENING`; grouping mismatch 목적은 `NEED-DATA`, blank/no-auto-fill, stale/invalid historical retention, zero drive/worker/link/dispatch I/O | live FC readback/compare, Simplified Format, FC9..12, OF14, Display Units, Apply/Revert/SV/import/export와 Motion/Recorder/Status unit propagation |
| Expert Limits / Protections · Documented Parameter Map v0.1 | 여섯 번째 LOCAL page에서 Current Limits 7개, Motion Limits/Modulo 9개 명령 row, Protections 11개 명령 row와 9개 문서 충돌·고정 danger warning을 immutable catalog로 탐색 | **로컬 catalog만 GREEN**; `DOCUMENTED_PARAMETER_MAP_ONLY · PARTIAL_NEED_DATA`, focused **69 passed**, 전체 **1513 passed**, 독립 closeout 잔여 finding 없음, Python 3.14 runtime 확인. 20개 frozen source identity, zero drive read/validation/evaluation/command/write/Apply/SV/unit propagation | current drive config와 active protection state, firmware/EAS parity, 값 유효성·추천, protection efficacy, read/write order·readback·rollback·Revert/SV와 현장 safety는 `NEED-DATA / NO-GO` |
| Expert Application Settings · Documented Map v0.1 | 일곱 번째 LOCAL page에서 Brake 4개, Settling Window 4개, Inputs/Outputs 5개 행을 immutable catalog로 탐색; I/O status semantics 2개는 `unavailable · not sampled` | **로컬 catalog/UI만 GREEN**; `DOCUMENTED_APPLICATION_SETTINGS_MAP_ONLY · PARTIAL_NEED_DATA`, focused **85 passed**, 전체 **1529 passed**, 독립 closeout 잔여 finding 없음, Python 3.14 runtime 확인. 24 frozen source identity, 9 conflicts, 16 warnings, 6 missing; zero drive read/worker/command/write/Apply/Revert/SV/output actuation/motion | current drive config/I/O state, exact B01G output electrical/brake capability, current/default 판정, transaction/readback/rollback, brake/safety efficacy와 field behavior는 `NEED-DATA / NO-GO` |
| Expert Hidden Verification – Bode · Documented Map v0.1 | 여덟 번째 `BODE DOC MAP` LOCAL page에서 Tuner Verification Settings, Current Bode, Velocity/Position Bode를 각 8개 row의 immutable catalog로 탐색; hidden-page visibility, field control과 actual Verify는 설명만 | **로컬 catalog/UI만 GREEN**; `DOCUMENTED_HIDDEN_BODE_MAP_ONLY · PARTIAL_NEED_DATA`, pure model **14 passed**, Expert 영향범위 **211 passed in 65.06s**, 전체 **1547 passed in 503.66s / exit 0**, Python 3.14 1366×820 OFFLINE runtime. 7 frozen source identity, 4 conflicts, 12 warnings, 8 missing; zero drive read/current-state observation/acquisition/evaluation/Verify/EAS-setting change/recording/command/write/Apply/Revert/SV/energization/motion. 기존 P1 Bode는 OFFLINE MODEL로 분리 | 현재 EAS setting/visibility와 target parity, automatic amplitude law, sample/recording provenance, abort/closeout, quantitative acceptance, actual Current ENERGY와 V/P MOTION field verification은 `NEED-DATA / NO-GO` |
| Expert Verification – Time · Documented Map v0.1 | 아홉 번째 `TIME DOC MAP` LOCAL page에서 Current Verification–Time, V/P Recording Setup, V/P Verification–Time을 각 8개 grouped row의 immutable catalog로 탐색; editable current, Recorder, Enable/PTP/Jog/Sine/Step과 control-parameter 위험은 설명만 | **로컬 catalog/UI만 GREEN**; `DOCUMENTED_TIME_VERIFICATION_MAP_ONLY · PARTIAL_NEED_DATA`, 전체 **1567 passed in 524.64s**, Python 3.14 1366×820 새 창 `OFFLINE · READ ONLY` runtime에서 8/8/8 groups와 실행 제어 0개 확인. 독립 검토 HIGH/MEDIUM/LOW 잔여 finding 없음, frozen source SHA-256 **8/8 일치**; zero drive/current/Recorder state read, Recorder configuration/acquisition, Verify, Enable, PTP, Jog, injection, command/write/Apply/SV, energization/motion | actual Current Verification–Time은 `ENERGIZING / NEED_DATA`이며 motor가 move/twitch할 수 있고, actual V/P Verification–Time은 editable current와 field weakening/friction compensation을 포함한 `MOTION / NEED_DATA`; waveform/current/travel/thermal/Recorder provenance, independent stop, abort/restore, quantitative acceptance 전에는 `NO-GO`. Summary transaction은 아래 별도 map으로 분리 |
| Expert Summary · Documented Transaction Map v0.1 | 열 번째 `SUMMARY DOC MAP` LOCAL page에서 Recommended Actions, Save Transaction, Authority Split을 각 4개 grouped row로 탐색; SV, drive upload, design export, motor DB import의 서로 다른 authority를 분리 | **로컬 catalog/UI만 GREEN**; `DOCUMENTED_SUMMARY_TRANSACTION_MAP_ONLY · PARTIAL_NEED_DATA`, 전체 **1587 passed in 266.87s**, Python 3.14 OFFLINE/READ ONLY runtime과 frozen source SHA-256 **3/3 일치**; zero drive read/upload, Save/SV, file dialog/export, design export, DB mutation, Apply, energization/motion | current EAS Summary state/recommendation, exact transaction order/atomicity, target/parameter identity, path/schema/integrity, readback/power-cycle, rollback/partial-failure와 actual save 결과는 `NEED-DATA / NO-GO` |
...
| Single Axis Enable / Disable v0.1 | 기존 Safety Snapshot으로 `MO/SO` lifecycle 4상태와 fault/unknown을 분리; locked Enable과 기존 STOP 경계를 표시 | OFFLINE MODEL; installed Gold `MO/SO/SR` command pages; zero-new-I/O | standalone `MO=1`, current envelope, operator/independent protection, telemetry abort, field closeout은 `NEED-DATA` |
| Single Axis Controls · Documented Authority Map v0.1 | Motion page의 별도 LOCAL frame에서 Status & I/O, Mode & Reference, Activation & Tools의 3 section/12 row를 immutable catalog로 탐색 | **로컬 catalog/UI만 GREEN**; `DOCUMENTED_SINGLE_AXIS_AUTHORITY_MAP_ONLY · PARTIAL_NEED_DATA`, 핵심 **53 passed in 45.33s**, 관련 영향 범위 **560 passed**, 전체 **1604 passed in 478.69s / exit 0 / stderr 0**, source SHA-256 **3/3 일치**, Python 3.14 QDD 1366×820 `OFFLINE · READ ONLY` runtime, 세 skin 수평 scroll 0·table contrast ≥4.5:1; zero worker/drive/terminal/Recorder I/O, action/edit widget 0 | identity-bound live state, Digital Output write, UM transaction, Enable/Disable, PTP/Jog/Current/Sine/Homing/Stepper, Terminal command, Recorder config/acquisition과 Gold family/EAS parity는 `NEED-DATA / NO-GO` |
| Single Axis Digital Inputs · Read-Only Snapshot v0.1 | 명시적 refresh에서 `IL[1..6]`, `IF[1..6]`, 최종 `IP`만 읽고 6개 drive logical state/function/polarity/sticky/filter를 동일 connection session의 bounded snapshot으로 표시 | **CURRENT TARGET READBACK OBSERVED / physical behavior UNVERIFIED**; input focused **133 passed in 96.03s / exit 0**, 최신 전체 repository **1673 passed in 493.02s / exit 0**, source SHA-256 **4/4 일치**. 2026-07-19 Read Only session에서 6/6 `ACTIVE · DRIVE LOGICAL`, General purpose, ACTIVE_HIGH, non-sticky, 0.000 ms, 25.9 ms acquisition을 관찰; no `IB`, assignment, Digital Output, mapping/filter change, enable/motion | EAS same-moment parity, physical pin/wiring response, polarity/filter/sticky timing 검증. 다른 Gold/I/O count 일반화와 safety interlock 사용은 `NEED-DATA / NO-GO` |
| Single Axis Digital Outputs · Read-Only Snapshot v0.1 | 명시적 refresh에서 `OL[1..4]`, `GO[1..4]`, 최종 `OP`만 읽고 Gold Twitter 네 출력의 drive logical activation/function/polarity/route를 동일 connection session의 bounded snapshot으로 표시 | **CURRENT TARGET READBACK OBSERVED / physical behavior UNVERIFIED**; pure contract **24 passed**, integration slice **113 passed**, 직접 영향 범위 **276 passed in 53.27s / exit 0**, 전체 **1673 passed in 493.02s / exit 0**, source SHA-256 **4/4 일치**. 2026-07-19 `ONLINE · READ ONLY`에서 4/4 `INACTIVE · DRIVE LOGICAL ACTIVATION`, General purpose, ACTIVE_HIGH, Function via OL[N], 18.1 ms acquisition을 관찰; no assignment, `OB/OC/XO`, output toggle/actuation, enable/motion | EAS same-moment parity, physical pin voltage/current, external load/brake behavior, output compare/STO indication 자극, write/readback/rollback. 다른 Gold output topology 일반화와 safety interlock 사용은 `NEED-DATA / NO-GO` |
| Single Axis Drive Mode · Read-Only Snapshot v0.1 | 명시적 refresh에서 `UM` 한 건만 읽고 installed Gold 문서의 exact `1/2/3/5/6` mode map으로 현재 mode와 reference consequence를 표시 | **CURRENT TARGET READBACK OBSERVED / MODE CHANGE NOT IMPLEMENTED**; pure **24 passed**, initial integration **261 passed in 88.22s**, latest focused split **53 + 287 passed**, 3-skin UI **32 passed in 57.09s**, 전체 **1717 passed in 1012.58s / exit 0**, source SHA-256 **1/1 일치**. 2026-07-19 `ONLINE · READ ONLY`, motor disabled/velocity 0/current 0에서 `UM=5 · Position`, 2.1 ms acquisition을 관찰; no `UM=` assignment, enable/reference/motion/SV | EAS same-moment parity, non-volatile UM transaction, disabled/stationary precondition, exact readback, persistence recovery, rollback, family/personality compatibility는 `NEED-DATA / NO-GO` |
| Single Axis Current Reference · Read-Only Snapshot v0.1 | 명시적 refresh에서 `MO/SO/MF/SR` pre/post와 `UM/TC/IQ/ID/CL[1]/PL[1]/LC/MC`만 읽고 상태·limit·motor-off current invariant를 identity-bound snapshot으로 교차검증 | **CURRENT TARGET READBACK OBSERVED / TC COMMAND NOT IMPLEMENTED**; initial pure/UI/catalog/safety 회귀와 transport RED→GREEN 뒤 최신 영향 범위 **286 passed in 108.92s / exit 0**, transport/telemetry safety **64 passed**, 전체 repository **1781 passed in 1269.31s / exit 0**, installed source SHA-256 **9/9 일치**. 2026-07-19 `ONLINE · READ ONLY`, `DISABLED REPORTED`, `UM=5 · Position`, `TC/IQ/ID=0 A`, `CL[1]=21.2132 A`, `PL[1]=70.7107 A`, `LC=OFF`, `MC=140.0000 A`, acquisition **32.2 ms** 관찰; no assignment, enable, loop change, energization, motion | EAS same-moment parity, native-current unit/RMS/torque interpretation, physical current sensing, thermal/current/torque envelope, independent protection, watchdog, abort/closeout 및 executable `TC=`는 `NEED-DATA / NO-GO` |
| Single Axis Position / Velocity References · Read-Only Snapshot v0.1 | 명시적 refresh에서 `MO/SO/MF/SR` pre/post와 `UM/PA[1]/PR[1]/JV/SP[1]/AC[1]/DC/SD/PX/VX`만 읽는다. PA/PR/JV는 configured/queued readback으로 표시하며 active command나 motion proof로 해석하지 않는다 | **CURRENT TARGET READBACK OBSERVED / COMMAND NOT IMPLEMENTED**; pure **56 passed**, transport+pure **144 passed**, authority/UI/catalog/transport 직접 영향 범위 **244 passed**, closeout 영향 범위 **270 passed in 104.51s**, 전체 **1868 passed in 735.79s**, installed source SHA-256 **13/13 일치**. 실제 1366×820 검증에서 마지막 VX 행 clipping을 RED로 재현해 table 420 px로 수정했다. 2026-07-19 `ONLINE · READ ONLY`, `DISABLED REPORTED`, `UM=5 Position`, PA/PR/JV=0, SP=4,444,444 cnt/s, AC/DC/SD=1,000,000 cnt/s², PX=-2,038,379,934 cnt, VX=0.000 cnt/s, acquisition **35.6 ms** 관찰; no assignment, `BG`, enable, energization, motion | EAS same-moment parity, user units/direction, coordinate validity, travel/speed/acceleration/stopping envelope, limit input, independent STO/E-stop, watchdog, abort/closeout 및 executable PA/PR/JV+BG는 `NEED-DATA / NO-GO` |
...
- Digital Input은 별도 `axis.digital_inputs.refresh` operation에서 입력 1–6의
  `IP/IL/IF`만, Digital Output은 `axis.digital_outputs.refresh`에서 Gold Twitter
  출력 1–4의 `OL/GO/OP`만 명시적으로 읽는다. `IB` sticky clear,
  input mapping/filter 변경, output assignment/actuation은 계속 잠근다.
- Drive Mode는 별도 `axis.drive_mode.refresh`에서 `UM` 한 건만 읽는다.
  `axis.drive_mode.change`는 non-volatile assignment, motor-off,
  exact readback, persistence recovery와 rollback 계약이 고정될 때까지
  `NEED-DATA`다.
- Current/Stepper/Sine Reference와 Terminal은 계속
  `eas.single_axis.*` operation으로 분리해 `NEED-DATA` 잠금을 유지한다.

## 다음 구현 순서

1. Expert v2 P1→P2 순수 OFFLINE 모델의 전체 회귀·독립 리뷰·runtime smoke를 유지.
2. User Units documented formula는 explicit-input `PARTIAL/SCREENING`과
   grouping-mismatch `PURPOSE NEED-DATA`를 유지하고 drive/other-page 단위로 전파하지 않음.
3. Limits/Protections documented map은 immutable catalog만 GREEN으로 유지하고
   current config·EAS parity·protection efficacy·write/safety로 승격하지 않음.
4. Application Settings documented map도 immutable catalog/UI만 GREEN으로 유지하고
   current I/O/brake state, transaction, output actuation 또는 safety로 승격하지 않음.
5. Hidden Verification–Bode도 static 문서 map만 GREEN으로 유지하고 page visibility,
   EAS settings, model/measurement parity, acquisition/recording 또는 actual
   ENERGY/MOTION Verify로 승격하지 않음.
6. Expert filter/scheduling은 현재처럼 `NEED-DATA`를 유지하며 공개 근거를 대조해
   명시적 입력·단위·범위가 고정된 항목만 구현.
7. EAS 미연결 세부 화면과 operation catalog 구현/잠금 상태를 항목별 대조.
8. Digital Input 1–6과 Gold Twitter Digital Output 1–4 read-only snapshot은
   current target query/readback을 보존한다. physical I/O/EAS parity,
   output actuation, 수동 reference·Terminal은 mapping/limit/watchdog/rollback
   근거 전까지 `NEED-DATA / NO-GO`를 유지한다.
9. Drive Mode는 current-target `UM=5` one-query observation으로 유지하고
   same-moment EAS parity와 비휘발성 transaction/rollback 근거 전까지
   변경 UI·worker job·assignment를 만들지 않는다.
10. 현장 gate가 준비된 개별 동작만 별도 승인 후 제한 검증.
