...
| Motion - Single Axis | Position/position error/velocity/active current/last fault/status/program status; Digital In/Out; `STO1/STO2/ERR`; Drive Mode(UM); Enable; `Current / Stepper / Sine Reference`; PTP Absolute/Relative; Terminal/Command Reference; 2-chart Recorder | core telemetry, session zero, locked finite PTP, 별도 Recorder와 `MO/SO/MF/PS/SR/MS` 기반 Safety Snapshot MODEL projection 구현. `Documented Authority Map v0.1`은 3 section/12 row에서 각 control의 risk를 inspect-only로 분리한다. Digital Input 1–6은 새 explicit read-only snapshot에서 `IL/IF/IP`만 읽는다. Digital Output, sticky clear, mapping/filter write, mode별 수동 구동, terminal, EAS recorder docking parity는 계속 미구현/NEED-DATA |
...
| Single Axis Digital Inputs · Read-Only Snapshot v0.1 | 명시적 refresh에서 `IL[1..6]`, `IF[1..6]`, 최종 `IP`만 읽고 6개 drive logical state/function/polarity/sticky/filter를 동일 connection session의 bounded snapshot으로 표시 | **CURRENT TARGET READBACK OBSERVED / physical behavior UNVERIFIED**; 초기 집중 **127 passed in 60.34s**, 확대 영향범위 **489 passed in 370.16s**(이후 field에서 발견한 worker observer allowlist 누락은 별도 RED→GREEN으로 수정), source SHA-256 **4/4 일치**. 2026-07-19 Read Only session에서 6/6 `ACTIVE · DRIVE LOGICAL`, General purpose, ACTIVE_HIGH, non-sticky, 0.000 ms, 25.9 ms acquisition을 관찰; no `IB`, assignment, Digital Output, mapping/filter change, enable/motion | 최신 수정 포함 전체 회귀, EAS same-moment parity, physical pin/wiring response, polarity/filter/sticky timing 검증. 다른 Gold/I/O count 일반화와 safety interlock 사용은 `NEED-DATA / NO-GO` |
...
- Digital Input은 별도 `axis.digital_inputs.refresh` operation에서 입력 1–6의
  `IP/IL/IF`만 명시적으로 읽는다. Digital Output, `IB` sticky clear,
  mapping/filter 변경, Current/Stepper/Sine Reference와 Terminal은 계속
...
8. Digital Input 1–6 read-only snapshot은 current target에서 query/readback을 검증하고,
   Digital Output·수동 reference·Terminal은 mapping/limit/watchdog/rollback 근거 전까지
   `NEED-DATA` 유지.
