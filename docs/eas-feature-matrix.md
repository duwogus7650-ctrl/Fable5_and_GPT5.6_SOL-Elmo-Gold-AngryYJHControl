...
| Expert 세부 | `Current Limits`; `Motion Limits and Modulo`; `Protections`; `Settling Window`; `Inputs and Outputs`; Current `Identification / Design / Verification-Time`; Commutation; Velocity/Position `Identification / Design / Scheduling / Verification-Time`; Summary | P2 v2는 explicit `K_a/B`, count/s·peak-A basis, `GS[2]=0` single-point MODEL을 구현. Filter/Scheduling v0.1은 documented type/location/mode/table topology만 LOCAL inspector로 제공. User Units, protection/limit 편집, I/O, settling, exact filter/scheduling evaluation, time-domain verification, summary 및 EAS import/export는 아직 parity가 아님 |
