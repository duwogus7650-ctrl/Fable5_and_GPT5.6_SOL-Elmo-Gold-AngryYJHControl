...
> 우선한다. EAS Single Axis 표시=`PU`, raw=`PX`이며 현재 target에서
> `PU-PX=2^25`다. Current Reference readback과 EAS형 5-preset local draft는
> 분리했고 모든 Set TC 출력은 잠겨 있다. 이 문서의 과거 `LIVE/완료` 표시는
> full EAS parity를 뜻하지 않는다.
...
| Single Axis Current Reference + EAS Preset Drafts · v0.2 | identity-bound `TC/IQ/ID/CL[1]/PL[1]/LC/MC` readback과 별도로 EAS live에서 관찰한 five host presets→same `TC` shape를 local-only model/UI로 제공한다 | **CURRENT TARGET READBACK OBSERVED / PRESET SHAPE PARTIAL / OUTPUT LOCKED**; EAS에서 5개 zero default와 `[TC]` tooltip, motor-off Set disabled 관찰. 앱은 5개 local draft와 CL/PL/MC warning만 제공하고 Set은 항상 disabled, signal/worker/drive I/O 0. 기존 readback 값·source evidence는 유지 | 실제 EAS Set/enable behavior, native-current unit/RMS/torque, physical current/thermal/torque envelope, independent protection, watchdog, abort/closeout 및 executable `TC=`는 `NEED-DATA / NO-GO` |
| Single Axis Position / Velocity References · Read-Only Snapshot v0.2 | `MO/SO/MF/SR` pre/post와 `UM/PA[1]/PR[1]/JV/SP[1]/AC[1]/DC/SD/PX/PU/XM/FC/CA[45]/VX`를 읽고 raw PX와 EAS/DS402 PU를 별도 authority로 표시한다 | **PX/PU VALUE PARITY OBSERVED / COMMAND NOT IMPLEMENTED**; EAS raw `PX=-2038379934`, `PU=-2004825502`, Single Axis display=`PU`, `PU-PX=2^25`, XM=0, FC=1, CA[45]=1. 앱은 delta를 자동 보정하지 않는다. installed source SHA-256 **17/17 일치**, 신규 집중 **142 passed**, 직접 영향 **284 passed**, ledger **5 passed**, 전체 **1956 passed in 692.83s / exit 0**; no assignment, `BG`, enable, energization, motion | `PU-PX` firmware-internal 원점, user units/direction, travel/speed/acceleration/stopping envelope, limit input, independent STO/E-stop, watchdog, abort/closeout 및 executable PA/PR/JV+BG는 `NEED-DATA / NO-GO` |
