...
| Motion - Single Axis | Position/position error/velocity/active current/last fault/status/program status; Digital In/Out; `STO1/STO2/ERR`; Drive Mode(UM); Enable; `Current / Stepper / Sine Reference`; PTP Absolute/Relative; Terminal/Command Reference; 2-chart Recorder | core telemetry, session zero, locked finite PTP, 별도 Recorder와 `MO/SO/MF/PS/SR/MS` 기반 Safety Snapshot MODEL projection 구현. `Documented Authority Map v0.1`은 3 section/12 row에서 각 control의 risk를 inspect-only로 분리한다. Digital Input 1–6은 explicit read-only `IL/IF/IP`, Gold Twitter Digital Output 1–4는 explicit read-only `OL/GO/OP` snapshot으로 구현했다. sticky clear, mapping/filter write, output assignment/actuation, mode별 수동 구동, terminal, EAS recorder docking parity는 계속 미구현/NEED-DATA |
...
- Digital Input은 별도 `axis.digital_inputs.refresh` operation에서 입력 1–6의
  ...
  input mapping/filter 변경, output assignment/actuation,
...
9. 현장 gate가 준비된 개별 동작만 별도 승인 후 제한 검증.
