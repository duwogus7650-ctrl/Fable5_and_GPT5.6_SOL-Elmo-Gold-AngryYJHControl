...
| Single Axis Digital Inputs · Read-Only Snapshot v0.1 | 명시적 refresh에서 `IL[1..6]`, `IF[1..6]`, 최종 `IP`만 읽고 6개 drive logical state/function/polarity/sticky/filter를 동일 connection session의 bounded snapshot으로 표시 | **CURRENT TARGET READBACK OBSERVED / physical behavior UNVERIFIED**; 최신 focused **133 passed in 96.03s / exit 0**, 전체 stdout **1639 passed in 827.24s / 100% / stderr 0**(전체 numeric exit watcher는 빈 값이므로 그 숫자만 `UNVERIFIED`), source SHA-256 **4/4 일치**. 2026-07-19 Read Only session에서 6/6 `ACTIVE · DRIVE LOGICAL`, General purpose, ACTIVE_HIGH, non-sticky, 0.000 ms, 25.9 ms acquisition을 관찰; no `IB`, assignment, Digital Output, mapping/filter change, enable/motion | EAS same-moment parity, physical pin/wiring response, polarity/filter/sticky timing 검증. 다른 Gold/I/O count 일반화와 safety interlock 사용은 `NEED-DATA / NO-GO` |
