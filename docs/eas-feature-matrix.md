...
| Expert Tuning | Axis; Motor/Feedback; User Units; Limits/Protections; Application Settings; Current; Commutation; Velocity/Position; Summary | v2는 Current P1 → Velocity/Position P2 순수 offline Candidate Lab과 documented Evidence, Local Page Status, User Units formula, Limits/Protections 및 Application Settings documented map의 일곱 단계를 제공한다. 공통 P1/commutation/P2/Verify, candidate/installed readback과 EAS Enter/Apply state는 분리하며 아래 세부 차이는 backlog |
| Expert 세부 | `Current Limits`; `Motion Limits and Modulo`; `Protections`; `Brake`; `Settling Window`; `Inputs and Outputs`; Current `Identification / Design / Verification-Time`; Commutation; Velocity/Position `Identification / Design / Scheduling / Verification-Time`; Summary | P2 v2는 explicit `K_a/B`, count/s·peak-A basis, `GS[2]=0` single-point MODEL을 구현. Filter/Scheduling v0.1은 documented topology만, Page Status v0.1은 기존 local object 상태만 inspector로 제공. User Units v0.1은 DS-402 위치 식 하나의 explicit-input `PARTIAL/SCREENING`이다. Limits/Protections와 Application Settings v0.1은 static documented map과 충돌/위험/누락만 제공한다. current-drive/I/O/brake readback, 값 검사·추천, protection efficacy, exact transaction/write/actuation, exact filter evaluator, time-domain verification, Summary recommendation, EAS page icon/Enter/Apply/import/export parity는 미구현 |
...
5. Expert filter/scheduling은 현재처럼 `NEED-DATA`를 유지하며 공개 근거를 대조해
   명시적 입력·단위·범위가 고정된 항목만 구현.
6. EAS 미연결 세부 화면과 operation catalog 구현/잠금 상태를 항목별 대조.
7. Digital I/O·수동 reference·Terminal은 mapping/limit/watchdog/rollback 근거 전까지
   `NEED-DATA` 유지.
