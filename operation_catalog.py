...
_SPECS = (
    ...
    _spec("tuning.p1.apply", "Apply P1 → Drive RAM", OperationRisk.RAM_WRITE,
          "Apply the current-loop candidate gains to drive RAM (motor OFF); the "
          "original set is read back and stored first and Restore rolls it back. No SV.",
    ...
    _spec("tuning.p1.save", "Save P1 → SV (LOCKED)",
          ...
          status=OperationStatus.NEED_DATA),
    _spec("tuning.p2.apply", "Apply P2 → Drive RAM", OperationRisk.RAM_WRITE,
          "Apply the velocity/position candidate gains to drive RAM (motor OFF); the "
          "original set is read back and stored first and Restore rolls it back. No SV.",
          gates=_SCOPED_MUTATION | frozenset(("trial_capability",))),
